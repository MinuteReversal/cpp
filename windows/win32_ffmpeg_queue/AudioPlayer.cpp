...
namespace nv {
  AudioPlayer::AudioPlayer(WORD nChannels_, DWORD nSamplesPerSec_)
      : nChannels(nChannels_), nSamplesPerSec(nSamplesPerSec_), pwfx(nullptr),
        flags(0) {
    Init();
  }

  HRESULT AudioPlayer::Start() { return pAudioClient->Start(); }

  HRESULT AudioPlayer::Stop() { return pAudioClient->Stop(); }
  ...
  HRESULT AudioPlayer::WriteFLTP(float* left, float* right,
                                 UINT32 sampleCount) {
    UINT32 padding;
    pAudioClient->GetCurrentPadding(&padding);
    if ((maxSampleCount - padding) <
        sampleCount) { // 音频写入太快了，超出缓冲区，我们直接清空现有缓冲区，保证时间对的上
      pAudioClient->Stop();
      pAudioClient->Reset();
      pAudioClient->Start();
    }
  ...
  HRESULT AudioPlayer::Init() {
    ...
    hr = pDevice->Activate(__uuidof(IAudioSessionManager), CLSCTX_INPROC_SERVER,
                           NULL, (void**)&pAudioSessionManager);

    CComPtr<IAudioSessionControl> pAudioSession;
    hr = pAudioSessionManager->GetAudioSessionControl(&GUID_NULL, FALSE,
                                                      &pAudioSession);
    ...
    pwfx->nAvgBytesPerSec =
        pwfx->nSamplesPerSec * 2 * (pwfx->wBitsPerSample / 8);
    // 必须使用这种格式
    ...
    hr = pAudioClient->Initialize(
        ...
        AUDCLNT_STREAMFLAGS_AUTOCONVERTPCM |
            AUDCLNT_STREAMFLAGS_SRC_DEFAULT_QUALITY, // 这里的flag告诉系统需要重采样

...
public:
  ...
  void DrawHand(HDC hdc, int nLength, int nWidth, int nDegrees,
                COLORREF clrColor);
...
void MainWindow::DoubleBuffering(HDC hdc_old) {
  ...
  //创建内存兼容位图hBmp
  HBITMAP hBmp =
      CreateCompatibleBitmap(hdc_old, clientRect.right, clientRect.bottom);
  //将内存位图选入缓冲内存DC中——以便可以绘制多个位图
  SelectObject(hdc, hBmp);
  //如果不执行这两步, 窗口显示出来会出现黑色背景
  // SelectObject(hdc, GetSysColorBrush(COLOR_3DFACE)); //设置刷子颜色 -
  // Rectangle()的填充色 由于 Rectangle() 画出有黑色边框线, 这里故意从 -1,-1
  //绘制 +2, +2, 这样黑色边框线超出可视范围,就看不到了
  Rectangle(hdc, -1, -1, clientRect.right + 1,
            clientRect.bottom + 1); //画窗体的整个背景
...
void MainWindow::DrawClockFace(HDC hdc) {
  ...
  for (size_t i = 0; i < 12; i++) {
    ...
    Ellipse(hdc, offsetLeft + x - HALFSIZE, offsetTop + y - HALFSIZE,
            offsetLeft + x + HALFSIZE, offsetTop + y + HALFSIZE);
  }
}
 ...
 * 画指针
 */
void MainWindow::DrawHand(HDC hdc, int nLength, int nWidth, int nDegrees,
                          COLORREF clrColor) {

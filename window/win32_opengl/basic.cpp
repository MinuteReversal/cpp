...
HWND CreateOpenGLWindow(PCWSTR title, int x, int y, int width, int height, BYTE type, DWORD flags)
{
  int pf;
  HDC hDC;
  HWND hWnd;
  ...
  hWnd = CreateWindow(L"OpenGL", title, WS_OVERLAPPEDWINDOW | WS_CLIPSIBLINGS | WS_CLIPCHILDREN, x, y, width, height, NULL, NULL, hInstance, NULL);
  ...
  if (pf == 0)
  {
    MessageBox(hWnd, L"ChoosePixelFormat() failed: Cannot find a suitable pixel format.", L"Error", MB_OK);
    return 0;
  }
